...
-- Con esto, los tipos GEOMETRY y GEOGRAPHY quedan disponibles, junto con funciones como ST_Area(), ST_Intersection(), ST_AsGeoJSON(), etc.
...
-- MULTIPOLYGON porque la vectorización producirá múltiples polígonos discontinuos (manchas de degradación esparcidas por Catalunya).
-- 4326 es el EPSG code de WGS84, el mismo CRS que usamos en GEE.
-- Guardar el CRS en la definición de columna permite a PostGIS reproyectar automáticamente si algún día se consulta con otro CRS.
...
-- Sin este índice, una consulta "dame zonas dentro de este bounding box" recorre TODA la tabla. Con el índice GIST, usa un árbol espacial (R-tree) y es órdenes de magnitud más rápida.
